...
// addition 3
bool priority_comp(const struct list_elem* left,
...
int float_min_int(int f, int i)
{
	return f - (i << 14);
}
int float_mul_int(int f, int i)
{
	return f * i;
}
...
int float_min_float(int f1, int f2)
{
	return f1 - f2;
}
...
thread_set_nice (int nice)
{
  ...
  struct thread* t = thread_current();
  t->nice = nice;
  t->priority = float_min_float(
		  float_min_float(
			  float_add_int(0, PRI_MAX),
			  float_div_int(t->recent_cpu, 4)),
		  float_mul_int(float_add_int(0, t->nice), 2))
	  >> 14;
...
thread_get_load_avg (void) 
{
  ...
  // addition 3
  return float_mul_int(load_avg, 100) >> 14;
}
...
thread_get_recent_cpu (void) 
{
  ...
  // addition 3
  return float_mul_int(thread_current()->recent_cpu, 100) >> 14;
}
...
void update_load_avg_and_recent_cpu(void)
{
	...
	load_avg = float_div_int(
			float_add_int(
				float_mul_int(load_avg, 59),
				ready_threads),
			60);
	...
	for(e = list_begin(&all_list); e != list_end(&all_list); e = list_next(e))
	{
		...
		if(t != idle_thread)
		{
			t->recent_cpu = float_add_int(
					float_mul_float(
						float_div_float(
							float_mul_int(load_avg, 2),
							float_add_int(float_mul_int(load_avg, 2), 1)),
						t->recent_cpu),
					t->nice);
		}
	}
	...
	for(e = list_begin(&all_list); e != list_end(&all_list); e = list_next(e))
	{
		...
		t->priority = float_min_float(
				float_min_float(
					float_add_int(0, PRI_MAX),
					float_div_int(t->recent_cpu, 4)),
				float_mul_int(float_add_int(0, t->nice), 2)) >> 14;
...
running_thread (void) 
{
  ...
  asm ("mov %%esp, %0" : "=g" (esp));
  return pg_round_down (esp);int float_add_int(int f, int i)
{
	return f + (i << 14);
}
int float_min_int(int f, int i)
